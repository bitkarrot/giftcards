      ...
      <q-card class="q-pa-lg q-pt-xl lnbits__dialog-card">
        <q-form @submit="createGiftCard">
          <div v-if="!createDialog.result" class="q-gutter-md">
            ...
            <!-- Fee Reserve Section -->
            <q-separator class="q-my-md"></q-separator>
            <h6 class="text-subtitle1 q-my-none">Redemption Fee</h6>
            <q-select
              filled
              dense
              emit-value
              map-options
              v-model="createDialog.data.fee_mode"
              :options="[
            ...
            ></q-select>
            <q-input
              v-if="createDialog.data.fee_mode === 'percentage'"
              filled
              dense
              v-model.number="createDialog.data.fee_percent"
              type="number"
              label="Fee Percentage (%)"
              hint="Max routing fee as % of card amount. E.g. 5 = 5% of amount."
              :rules="[
                val => val >= 0 || 'Fee percentage must be >= 0'
              ]"
            >
              <template v-slot:append>
            ...
            <q-input
              v-if="createDialog.data.fee_mode === 'manual'"
              filled
              dense
              v-model.number="createDialog.data.fee_sats"
              type="number"
              label="Fee (sats)"
              hint="Fixed max routing fee in sats. Reserved from your wallet on top of the card amount."
              :rules="[
                val => val >= 0 || 'Fee must be >= 0 sats'
              ]"
            >
              <template v-slot:append>
                <span class="text-grey">sats</span>
              </template>
            </q-input>
              ...
              <q-tab-panel name="same">
                <div class="q-gutter-md">
                  ...
                  <q-select
                    ...
                    emit-value
                    map-options
                    v-model="bulkDialog.sameData.fee_mode"
                    :options="[
                      {label: 'Default (global setting)', value: 'default'},
                      {label: 'Percentage of amount', value: 'percentage'},
                      {label: 'Manual (fixed sats)', value: 'manual'}
                  ...
                  ></q-select>
                  <q-input
                    v-if="bulkDialog.sameData.fee_mode === 'percentage'"
                    filled
                    dense
                    v-model.number="bulkDialog.sameData.fee_percent"
                    type="number"
                    label="Fee Percentage (%)"
                    :rules="[val => val >= 0 || 'Must be >= 0']"
                  >
                    <template v-slot:append><span class="text-grey">%</span></template>
                  </q-input>
                  <q-input
                    v-if="bulkDialog.sameData.fee_mode === 'manual'"
                    filled
                    dense
                    v-model.number="bulkDialog.sameData.fee_sats"
                    type="number"
                    label="Fee (sats)"
                    :rules="[val => val >= 0 || 'Must be >= 0']"
                  >
                    <template v-slot:append><span class="text-grey">sats</span></template>
                  </q-input>
      ...
      <q-card class="q-pa-lg q-pt-xl lnbits__dialog-card">
        <div class="q-gutter-md" v-if="detailDialog.card">
          ...
          <div class="row q-col-gutter-md">
            ...
            <div class="col-12 col-md-6">
              <div class="text-caption">Redemption Fee</div>
              <div class="text-body2">{{ formatFeeLabel(detailDialog.card) }}</div>
            </div>
          </div>
