...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - Test 6: GET /giftcards/api/v1/cards/public/{token_hash} returns a PublicGiftCard with status, amount, sender_name, message, and expires_at; it omits wallet, card_wallet_id, and token_hash.
  </behavior>
  <action>
...
<task type="auto" tdd="true">
  ...
  <action>
    ...
    Honor the locked decisions: use LNURL-withdraw as the redemption primitive (D-01); serve the web page at /redeem/{raw_token} and the LNURL entry point at /api/v1/lnurl/{token_hash} (D-02); keep public endpoints unauthenticated and rely on the unguessable hash for authorization, deferring rate limiting to Phase 6 (D-06, D-12); support states active, redeeming, redeemed, and expired (D-07 extended by the `redeeming` intermediate state required for atomic concurrency guard and failure recovery per D-15); namespace tables as giftcards.cards under `lnbits.db.Database("ext_giftcards")` (D-13); and store `token_hash` as a unique indexed column (D-14).
